...
/** Deterministic per-chunk spot picker — scans a chunk-seeded shuffle of the 4×4 Voronoi
 *  cell grid for the first pancake top whose biome matches the structure's filter. Used by
 *  compat mixins to align the spawn-check pos with the placement pos. */
public final class BeyondPancakeScan {
    ...
    /** One sample at the center of every Voronoi 2D cell in the chunk (cells are 4 wide,
     *  chunk-local x/z ∈ {2, 6, 10, 14}) so sparse biomes can't fall between samples. */
    ...
    /** Sentinel for "no valid spot exists in this chunk" — distinguishes from cache miss. */
    private static final int[] NO_SPOT = new int[0];

    private static final ConcurrentMap<Long, int[]> SPOT_CACHE = new ConcurrentHashMap<>();

    private BeyondPancakeScan() {}

    /** Picks a {@code (worldX, Y, worldZ)} spot in the chunk whose pancake-top biome is in
     *  {@code structureBiomes} (or in {@code #is_end} if {@code structureBiomes} is null).
     *  Cached per chunk; returns {@code null} if no matching spot exists. */
    ...
    /** Read-only cache access for mixin redirects that don't have the helpers' context. */
    ...
    /** Returns the cached spot's Y for {@code (worldX, worldZ)}, or {@code fallback} when
     *  Beyond is inactive or no spot exists. Single-line replacement for the
     *  Beyond-active+cache-read+spot-check pattern in {@code @WrapOperation} handlers. */
    ...
    /** Returns a {@link BlockPos} at the cached spot's (x, z) keeping the caller's y, or the
     *  caller's (x, y, z) when Beyond is inactive or no spot exists. */
    ...
    /** {@link BlockPos.MutableBlockPos#set(int, int, int)} variant that uses the cached spot's
     *  (x, z) when available. */
    ...
    /** SDF origin returned this many blocks below the pancake surface so the shape's lower
     *  blocks replace pancake interior — anchors the base firmly. */
    private static final int BURY_DEPTH = 3;
    /** Require at least this many of the 4 cardinal neighbours (±2 blocks at surface Y) to
     *  be solid so the trunk/dome isn't placed at a 1-tile shard. */
    ...
    /** Probes the 4 cardinal neighbours at ±2 blocks. Requires {@link #MIN_SOLID_NEIGHBOURS}
     *  to be solid at {@code surfaceY} so the SDF base/dome lands on a real island, not a
