package com.thebeyond.common.entity.util;
...
public class SlowRotMoveControl extends MoveControl {
    float maxRot;
    public SlowRotMoveControl(Mob mob, float maxRot) {
        super(mob);
        this.maxRot = maxRot;
    }
    ...
    public void tick() {
        if (this.operation == MoveControl.Operation.STRAFE) {
            ...
            float f9 = (float)(Mth.atan2(d1, d0) * (double)180.0F / (double)(float)Math.PI) - 90.0F;
            this.mob.setYRot(this.rotlerp(this.mob.getYRot(), f9, maxRot));
    ...
    private boolean isWalkable(float relativeX, float relativeZ) {
        ...
        if (pathnavigation != null) {
            ...
            if (nodeevaluator != null && nodeevaluator.getPathType(this.mob, BlockPos.containing(this.mob.getX() + (double)relativeX, (double)this.mob.getBlockY(), this.mob.getZ() + (double)relativeZ)) != PathType.WALKABLE) {
                return false;
            }
        }
