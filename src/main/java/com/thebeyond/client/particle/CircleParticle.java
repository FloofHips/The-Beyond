...
public class CircleParticle extends TextureSheetParticle {
    ...
    public CircleParticle(ClientLevel level, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed, CircleColorTransitionOptions options, SpriteSet sprites) {
        super(level, x, y, z, 0, 0, 0);
        this.sprites = sprites;

        this.quadSize = options.scale();
        this.lifetime = Math.min((int) (50 * options.scale()), 70);
        this.gravity = 0;
    ...
    @OnlyIn(Dist.CLIENT)
    public static class Provider implements ParticleProvider<CircleColorTransitionOptions> {
        private final SpriteSet sprites;
        ...
        @Override
        public Particle createParticle(CircleColorTransitionOptions options, ClientLevel level, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
            return new CircleParticle(level, x, y, z, xSpeed, ySpeed, zSpeed, options, this.sprites);
        }
    }
}
