...
@OnlyIn(Dist.CLIENT)
public class PrismographBlockScreen extends AbstractContainerScreen<PrismographMenu> {
    private static final int HUMP_W = 52;
    private static final int SPLIT = 32; // hump height above the panel

    public PrismographBlockScreen(PrismographMenu menu, Inventory playerInventory, Component title) {
    ...
    protected void renderBg(GuiGraphics g, float partialTick, int mouseX, int mouseY) {
        ...
        Slot film = this.menu.slots.get(PrismographBlockEntity.FILM);
        if (!film.hasItem()) {
